...
def spatial_singlet_filter(
    ...
    for i in range(iters):
        ...
        if mode == "lowmem":
            ...
            basis_distances = scisp.distance.squareform(scisp.distance.pdist(adata.obsm[basis]))
...
def rotate_embedding(adata, basis, theta, inplace=True):
    ...
    if basis not in adata.obsm_keys():
        raise ValueError(f"The specified embedding basis '{basis}' is not found in adata.obsm.")
    ...
    if inplace:
        ...
        # Create a new anndata object with rotated spatial coordinates
        adata_rotated = ad.AnnData(X=adata.X, obs=adata.obs, var=adata.var, uns=adata.uns, obsm={basis: rotated_coords})
        return adata_rotated


# Rescale spatial coordinates in an AnnData object
def rescale_embedding(adata, basis, scale_factor, inplace=True):
    """
    Rescale spatial coordinates in an anndata object.
    ...
    """
    if basis not in adata.obsm_keys():
        raise ValueError(f"The basis '{basis}' is not present in the 'obsm' of the anndata object.")

    # Get the spatial coordinates from the 'obsm' attribute
    coords = adata.obsm[basis]

    # Rescale the spatial coordinates
    scaled_coords = coords * scale_factor

    if inplace:
        # Update the 'obsm' attribute with the rescaled spatial coordinates
        adata.obsm[basis] = scaled_coords
        return None
    else:
        # Create a copy of the original anndata object and update its 'obsm' attribute with the rescaled spatial coordinates
        new_adata = adata.copy()
        new_adata.obsm[basis] = scaled_coords
        return new_adata
